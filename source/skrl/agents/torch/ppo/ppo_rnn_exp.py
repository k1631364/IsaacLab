...
class PPO_RNN_EXP(Agent):
    ...
    def init(self, trainer_cfg: Optional[Mapping[str, Any]] = None) -> None:
        ...
        if self.memory is not None:
            ...
            self.memory.create_tensor(name="phase", size=1, dtype=torch.bool)

            # tensors sampled during training
            # self._tensors_names = ["states", "actions", "terminated", "log_prob", "values", "returns", "advantages"]
            self._tensors_names = ["states", "actions", "terminated", "log_prob", "values", "returns", "advantages", "phase"]
    ...
    def filter_batch_by_phase(self, batch, phase_tensor):
        task_phase_indexes = (phase_tensor == False).nonzero(as_tuple=True)[0]
        # print("Task phsae indexes")
        # print(task_phase_indexes)
        filtered_batch = []
        for tensor in batch:
            filtered_batch.append(tensor[task_phase_indexes])
    ...
    def record_transition(self,
        ...
        # actions = actions[:20,:]
        # rewards = rewards[:20,:]
        # next_states = next_states[:20,:]
        # terminated = terminated[:20,:]
        ...
        # rewards[-10:,:] = float("nan")
        # next_states[-10:,:] = float("nan")
        # terminated[-10:,:] = float("nan")
        # truncated[-10:,:] = float("nan")
        ...
        if self.memory is not None:
            ...
            # print("Memorryyyyyy")
            # print(self.memory.get_tensor_by_name("states").shape)
            ...
            # print("State shappeeee")
            # print(states.shape)
            # print(self.memory.get_tensor_by_name("states").shape)
            # print(terminated.shape)
            # print(self.memory.get_tensor_by_name("terminated").shape)
            phase_mask = infos["phase"]["mask"].reshape((-1,1))
            # print(phase_mask.shape)
            self.memory.add_samples(states=states, actions=actions, rewards=rewards, next_states=next_states,
                                    terminated=terminated, truncated=truncated, log_prob=self._current_log_prob, values=values, phase=phase_mask, **rnn_states)
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        def compute_gae(rewards: torch.Tensor,
            ...
            # print("Reward shapeeeeee")
            # print(rewards.shape)
            # print(values.shape)
            # print(not_dones.shape)
            # print(discount_factor)
            # print(next_values.shape)
            # print(lambda_coefficient)
            # print(advantage)
            # rewards = rewards[:,:20,:]
            # values = values[:,:20,:]
            # not_dones = not_dones[:,:20,:]
            # advantages = advantages[:,:20,:]
        ...
        # print("sampled batch")
        # print(self._tensors_names)
        # print(self._mini_batches)        
        # print(sampled_batches[0][0])
        phase_index = self._tensors_names.index('phase')
        phase_tensor = sampled_batches[0][phase_index]
        # print(phase_tensor.shape)
        # states_index = self._tensors_names.index('states')
        # states_tensor = sampled_batches[0][states_index]
        # print(states_tensor.shape)

        filtered_batches = []
        for batch in sampled_batches:
            # Extract the phase tensor for this batch
            # phase_tensor = batch[phase_index]
            filtered_batches.append(self.filter_batch_by_phase(batch, phase_tensor))
        ...
        if self._rnn:
            ...
            filtered_rnn_batches = []
            for batch in sampled_rnn_batches:
                # Extract the phase tensor for this batch
                # phase_tensor = batch[phase_index]
                filtered_rnn_batches.append(self.filter_batch_by_phase(batch, phase_tensor))

        cumulative_policy_loss = 0
        cumulative_entropy_loss = 0
        cumulative_value_loss = 0

        # print(type(filtered_batches))
        # print(len(filtered_batches))
        # print(phase_tensor)
        if (phase_tensor == False).any(): 
            # learning epochs
            for epoch in range(self._learning_epochs):
                kl_divergences = []

                # mini-batches loop
                for i, (sampled_states, sampled_actions, sampled_dones, sampled_log_prob, sampled_values, sampled_returns, sampled_advantages, sampled_phases) in enumerate(filtered_batches):

                    if self._rnn:
                        if self.policy is self.value:
                            rnn_policy = {"rnn": [s.transpose(0, 1) for s in filtered_rnn_batches[i]], "terminated": sampled_dones}
                            rnn_value = rnn_policy
                        else:
                            rnn_policy = {"rnn": [s.transpose(0, 1) for s, n in zip(filtered_rnn_batches[i], self._rnn_tensors_names) if "policy" in n], "terminated": sampled_dones}
                            rnn_value = {"rnn": [s.transpose(0, 1) for s, n in zip(filtered_rnn_batches[i], self._rnn_tensors_names) if "value" in n], "terminated": sampled_dones}
