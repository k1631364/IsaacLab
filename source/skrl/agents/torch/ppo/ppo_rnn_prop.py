...
class PPO_RNN_PROP(Agent):
    def __init__(self,
                 ...
                 memory_all: Optional[Union[Memory, Tuple[Memory]]] = None,
    ...
    def init(self, trainer_cfg: Optional[Mapping[str, Any]] = None) -> None:
        ...
        # create tensors in memory
        if self.memory_all is not None:
            self.memory_all.create_tensor(name="states", size=self.observation_space, dtype=torch.float32)
            self.memory_all.create_tensor(name="actions", size=self.action_space, dtype=torch.float32)
            self.memory_all.create_tensor(name="rewards", size=1, dtype=torch.float32)
            self.memory_all.create_tensor(name="terminated", size=1, dtype=torch.bool)
            self.memory_all.create_tensor(name="log_prob", size=1, dtype=torch.float32)
            self.memory_all.create_tensor(name="values", size=1, dtype=torch.float32)
            self.memory_all.create_tensor(name="returns", size=1, dtype=torch.float32)
            self.memory_all.create_tensor(name="advantages", size=1, dtype=torch.float32)
            self.memory_all.create_tensor(name="props", size=3, dtype=torch.float32)
        ...
        for i, size in enumerate(self.policy.get_specification().get("rnn", {}).get("sizes", [])):
            ...
            if self.memory_all is not None:
                self.memory_all.create_tensor(name=f"rnn_policy_{i}", size=(size[0], size[2]), dtype=torch.float32, keep_dimensions=True)
            # default RNN states
        ...
        if self.value is not None:
            if self.policy is self.value:
                ...
                for i, size in enumerate(self.value.get_specification().get("rnn", {}).get("sizes", [])):
                    ...
                    if self.memory_all is not None:
                        self.memory_all.create_tensor(name=f"rnn_value_{i}", size=(size[0], size[2]), dtype=torch.float32, keep_dimensions=True)
                    # default RNN states
    ...
    def record_transition(self,
        ...
        if self.memory is not None:
            ...
            if self.memory_all is not None:
                # storage transition in memory
                # print("Moemory allllll")
                # print(infos["prop"].shape)
                self.memory_all.add_samples(states=states, actions=actions, rewards=rewards, next_states=next_states,
                                        terminated=terminated, truncated=truncated, log_prob=self._current_log_prob, values=values, props=infos["prop"], **rnn_states)
                for memory_all in self.secondary_memories:
                    memory_all.add_samples(states=states, actions=actions, rewards=rewards, next_states=next_states,
                                    terminated=terminated, truncated=truncated, log_prob=self._current_log_prob, values=values, props=infos["prop"], **rnn_states)
    ...
    def post_interaction(self, timestep: int, timesteps: int) -> None:
        ...
        # update and write memories
        if self.memory_all is not None:
            if timestep > 1 and self.checkpoint_interval > 0 and not timestep % self.checkpoint_interval:
                memory_path = os.path.join(self.experiment_dir, "memory_all")
                if os.path.exists(memory_path):
                    shutil.rmtree(memory_path)
                os.makedirs(memory_path)
                self.memory_all.save(memory_path)
